...
class UppaalCLanguageParser(Parser):
            ...
            def block2():
                with self._group():
                    with self._choice():
                        ...
                        with self._option():
                            self._ChanDefault_()
                        self._error('expecting one of: ChanDefault ChanExpr')
            self._positive_gather(block2, sep2)
        self._positive_gather(block1, sep1)
    ...
    @tatsumasu()
    def _PropUnderStrategy_(self):  # noqa
        self._PropPathQuant_()
        self.name_last_node('prop')
        self._token('under')
        self._ID_()
        self.name_last_node('strategy')
        self._constant('PropUnderStrategy')
        self.name_last_node('astType')
        self.ast._define(
            ['astType', 'prop', 'strategy'],
            []
        )

    @tatsumasu()
    def _Strategy_(self):  # noqa
        self._token('strategy')
        self._ID_()
        self.name_last_node('name')
        self._token('=')
        self._token('control')
    ...
    @tatsumasu()
    def _MinMaxObjective_(self):  # noqa
        self._token('strategy')
        self._ID_()
        self.name_last_node('name')
        self._token('=')
        with self._group():
            with self._choice():
                ...
                with self._option():
                    self._token('maxE')
                self._error('expecting one of: maxE minE')
        self.name_last_node('type')
        self._QExpression_()
        self.name_last_node('expr')
        self._token('[')
        ...
        self._ID_()
        self.name_last_node('strategy')
        self._constant('MinMaxObjective')
        self.name_last_node('astType')
        self.ast._define(
            ['astType', 'expr', 'name', 'prop', 'strategy', 'timeBound', 'type'],
            []
        )
    ...
    @tatsumasu()
    def _UppaalStrategoProp_(self):  # noqa
        with self._choice():
            with self._option():
                self._PropUnderStrategy_()
            with self._option():
                self._Strategy_()
            with self._option():
                self._MinMaxObjective_()
            self._error('expecting one of: ! ( + - /[ \\t\\r\\n]*/ /[-+]?[0-9]*\\.[0-9]+([Ee][-+]?[0-9]+)?/ /[-+]?[0-9]+/ /[a-zA-Z_][a-zA-Z0-9_]*/ A Boolean Double E ID Integer MinMaxObjective Number Predicate PropAll PropExists PropLeadsTo PropPathQuant PropUnderStrategy QBasicExpression QBinaryExpr QBracketExpr QDeadlock QExists QExpression QForAll QFuncCallExpr QSum QTernaryExpr QUnaryExpr Strategy Unary Value Variable _ deadlock exists false forall not strategy sum true')

    @tatsumasu()
    def _UppaalProp_(self):  # noqa
        with self._choice():
            with self._option():
                self._UppaalStrategoProp_()
            with self._option():
                self._UppaalTCTLProp_()
            with self._option():
                self._UppaalSMCProp_()
            self._error('expecting one of: ! ( + - /[ \\t\\r\\n]*/ /[-+]?[0-9]*\\.[0-9]+([Ee][-+]?[0-9]+)?/ /[-+]?[0-9]+/ /[a-zA-Z_][a-zA-Z0-9_]*/ A Boolean Double E E[ HypothesisTest ID Integer MinMaxObjective Number Pr Predicate ProbCompare ProbEstimate PropAll PropExists PropLeadsTo PropPathQuant PropUnderStrategy QBasicExpression QBinaryExpr QBracketExpr QDeadlock QExists QExpression QForAll QFuncCallExpr QSum QTernaryExpr QUnaryExpr Sim SimAcceptRuns Strategy SupInf Unary UppaalSMCProp UppaalStrategoProp UppaalTCTLProp Value ValueEstimate Variable _ deadlock exists false forall inf not simulate strategy sum sup true')
...
class UppaalCLanguageSemantics(object):
    ...
    def PropUnderStrategy(self, ast):  # noqa
        return ast

    def Strategy(self, ast):  # noqa
        return ast

    def MinMaxObjective(self, ast):  # noqa
        return ast
    ...
    def UppaalStrategoProp(self, ast):  # noqa
        return ast
