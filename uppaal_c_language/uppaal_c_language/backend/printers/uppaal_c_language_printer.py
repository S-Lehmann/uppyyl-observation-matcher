...
from uppaal_c_language.backend.helper import indent
...
class UppaalCPrinter(ASTCodePrinter):
    ...
    def ast_to_string(self, ast):
        ...
        if ast is None:
            return ""
        assert isinstance(ast, dict), f'The given AST is not a dictionary:\n{ast}'
...
def initialiser_array(printer, ast):
    ...
    val_strs = list(map(lambda var: printer.ast_to_string(var), ast["vals"]))
    if len(ast["vals"]) > 0 and ast["vals"][0]["astType"] == "InitialiserArray":  # Print sub-arrays in individual lines
        vals_str = ',\n'.join(map(lambda s: indent(s, 4), val_strs))
        return f'{{\n{vals_str}\n}}'
    else:
        chunk_size = 9
        if len(val_strs) > chunk_size:  # Print 9 entries per row if more than 9 entries exist
            line_chunks = [val_strs[i:i + chunk_size] for i in range(0, len(val_strs), chunk_size)]
            vals_str = ',\n'.join(map(lambda chunk: indent(','.join(chunk), 4), line_chunks))
            return f'{{\n{vals_str}\n}}'
        else:
