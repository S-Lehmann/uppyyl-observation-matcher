...
from uppaal_c_language.backend.ast_code_printer import (
    ASTCodePrinter
)
from uppaal_c_language.backend.printers.uppaal_c_language_printer import (
    UppaalCPrinter
)
...
def prop_under_strategy(printer, ast):
    """Prints a property query under strategy "phi under Strat"."""
    prop_str = printer.ast_to_string(ast["prop"])
    strategy_id_str = ast["strategy"]
    return f'{prop_str} under {strategy_id_str}'


def strategy(printer, ast):
    """Prints a strategy property query "strategy Strat = control: A[] ..."."""
    strategy_id_str = ast["name"]
    prop_str = printer.ast_to_string(ast["prop"])
    return f'strategy {strategy_id_str} = control: {prop_str}'


# def min_max_objective(printer, ast):
#     """Prints a min/max objective query "strategy Strat = minE ..."."""
#     pass
...
to_string_funcs = {
    ...
    "PropUnderStrategy": prop_under_strategy,
    "Strategy": strategy,
    # "MinMaxObjective": min_max_objective,
}
