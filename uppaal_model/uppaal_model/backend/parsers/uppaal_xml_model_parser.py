...
import uppaal_model.backend.models.nta.nta as nta
...
from uppaal_model.backend.models.base.query import Query
...
def uppaal_xml_to_dict(system_xml_str):
    """Transforms the XML description of an Uppaal system into a data dictionary.
    ...
    for template_element in template_elements:
        ...
        for location_element in location_elements:
            ...
            for label_element in label_elements:
                label = OrderedDict()
                label["id"] = unique_id("label")
                if "x" in label_element.attrib and "y" in label_element.attrib:
                    label["pos"] = {
                ...
                elif label_element.attrib["kind"] == "testcodeEnter":
                    location["testcode"]["enter"] = label_element.text
                elif label_element.attrib["kind"] == "testcodeExit":
                    location["testcode"]["exit"] = label_element.text
        ...
        for edge_element in edge_elements:
            ...
            edge["controllable"] = edge_element.attrib.get("controllable")
            ...
            for label_element in label_elements:
                ...
                if label_element.attrib["kind"] == "select":
                    edge["select"] = label_element.text
                    edge["select_label"] = label
                elif label_element.attrib["kind"] == "guard":
                ...
                elif label_element.attrib["kind"] == "testcode":
                    edge["testcode"]["trigger"] = label_element.text
    ...
    if root_query_element is not None:
        ...
        for query_element in query_elements:
            ...
            if formula_element is not None:
                query["formula"] = formula_element.text.strip() if (formula_element.text is not None) else ""

            comment_element = query_element.find("comment")
            if comment_element is not None:
                query["comment"] = comment_element.text.strip() if (comment_element.text is not None) else ""
...
def uppaal_dict_to_system(system_data):
    """Transforms the data dictionary of an Uppaal system into a system object.
    ...
    for template_data in system_data["templates"]:
        ...
        for location_data in template_data["locations"]:
            ...
            location.testcode = location_data["testcode"]
        ...
        for edge_data in template_data["edges"]:
            ...
            if edge_data["controllable"] == "true":
                edge.controllable = True
            elif edge_data["controllable"] == "false":
                edge.controllable = False
            else:
                edge.controllable = None
            ...
            edge.testcode = edge_data["testcode"]
...
def uppaal_xml_to_system(system_xml_str):
    """Transforms the XML description of an Uppaal system into a system object.
...
def uppaal_system_to_dict(system):
    ...
    for template_id, template in system.templates.items():
        ...
        for location_id, location in template.locations.items():
            ...
            # Parse location labels
            all_invariant_strs = list(map(lambda inv: inv.text, location.invariants))
            # if len(all_invariant_strs) > 1:
            #     all_invariant_strs[0] = (7 * " ") + all_invariant_strs[0]
            location_data["invariant"] = " &&\n".join(all_invariant_strs)
            location_data["invariant_label"] = deepcopy(location.view["invariant_label"])

            location_data["testcode"] = location.testcode
        ...
        for edge_id, edge in template.edges.items():
            ...
            if hasattr(edge, "controllable") and (edge.controllable is not None):
                edge_data["controllable"] = "true" if edge.controllable else "false"

            edge_data["source_loc_id"] = edge.source.id
            edge_data["target_loc_id"] = edge.target.id

            # Parse edge labels
            all_guard_strs = (list(map(lambda clock_grd: clock_grd.text, edge.clock_guards)) +
                              list(map(lambda variable_grd: variable_grd.text, edge.variable_guards)))
            # if len(all_guard_strs) > 1:
            #     all_guard_strs[0] = (7 * " ") + all_guard_strs[0]
            ...
            edge_data["testcode"] = edge.testcode
...
def uppaal_dict_to_xml(system_data):
    """Transforms the data dictionary of an Uppaal system into an XML description.
    ...
    for template_data in system_data["templates"]:
        ...
        for location_data in template_data["locations"]:
            ...
            if location_data["testcode"]:
                if location_data["testcode"]["enter"]:
                    testcode_label_element = etree.SubElement(location_element, "label", kind="testcodeEnter")
                    testcode_label_element.text = location_data["testcode"]["enter"]
                if location_data["testcode"]["exit"]:
                    testcode_label_element = etree.SubElement(location_element, "label", kind="testcodeExit")
                    testcode_label_element.text = location_data["testcode"]["exit"]
        ...
        ###############
        for edge_data in template_data["edges"]:
            if "controllable" in edge_data:
                edge_element = etree.SubElement(template_element, "transition", controllable=edge_data["controllable"])
            else:
            ...
            if edge_data["testcode"]:
                if edge_data["testcode"]["trigger"]:
                    testcode_label_element = etree.SubElement(edge_element, "label", kind="testcode")
                    testcode_label_element.text = edge_data["testcode"]["trigger"]
